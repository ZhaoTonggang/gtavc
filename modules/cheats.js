(() => {
    ...
    <div class="ce-section">
		...
		<div class="switch-container">
		    <span class="switch-label">空中飞行(右Shift)</span>
		    <label class="switch">
		...
		<div class="switch-container">
		    <span class="switch-label">上帝模式(无限生命)</span>
		    <label class="switch">
	...
	JSEvents.registerOrRemoveHandler = function(h) {
		if (h.handlerFunc && !h._wrapped) {
			...
			h.handlerFunc = function(e) {
				if (menuOpen && !e._isCheat && (h.eventTypeString.startsWith('key') || h.eventTypeString
						.startsWith('mouse') || h.eventTypeString.startsWith('touch'))) {
	...
	if (JSEvents.eventHandlers) {
		JSEvents.eventHandlers.forEach(h => {
			if (h.handlerFunc && !h._wrapped) {
				const originalHandler = h.handlerFunc;
				h.handlerFunc = function(e) {
					if (menuOpen && !e._isCheat && (h.eventTypeString.startsWith('key') || h
							.eventTypeString.startsWith('mouse') || h.eventTypeString.startsWith(
								'touch'))) {
						if (e && e.key === 'F3') return originalHandler.apply(this, arguments);
						return;
					}
	...
	window.typeCheat = async function(code) {
		...
		const handlers = JSEvents.eventHandlers.filter(h =>
			h.eventTypeString === 'keydown' || h.eventTypeString === 'keypress' || h.eventTypeString ===
			'keyup'
		);
		...
		for (let i = 0; i < code.length; i++) {
			...
			for (const h of handlers) {
				if (h.eventTypeString === 'keydown') getWasmTableEntry(h.callbackfunc)(h.eventTypeId,
					eventDataPtr, h.userData);
			}
			fillBuffer();
			for (const h of handlers) {
				if (h.eventTypeString === 'keypress') getWasmTableEntry(h.callbackfunc)(h.eventTypeId,
					eventDataPtr, h.userData);
			}
			fillBuffer();
			for (const h of handlers) {
				if (h.eventTypeString === 'keyup') getWasmTableEntry(h.callbackfunc)(h.eventTypeId,
					eventDataPtr, h.userData);
			}
	...
	function toggleGodMode(e) {
		...
		document.getElementById('ce-airbreak-status').textContent = godModeEnabled ? 'GodMode ON - Infinite HP' :
			...
			if (flyBtnDragging) {
				...
				if (airbreakConfigured) {
					...
					document.getElementById('ce-airbreak-status').textContent =
						...
						`Ready! [Tap FLY]`;
					document.getElementById('ce-airbreak-status').style.color = airbreakEnabled ? '#ff0' :
						'#0f0';
