...
(function(CloudSDK) {
	...
	async function pullFromStorage(token, fileName) {
		...
		if (profile) {
			try {
				const response = await fetch(API_PREFIX + "/saves/download/" + token + "/" + encodeURIComponent(
					fileName), {
					cache: "no-cache"
				});
				if (response.ok) {
...
(function(CloudSDKUI) {
			...
			bind: () => {
				if (v8Key === null) {
					...
					CloudSDK.resolveToken(v8Key).then((token) => {
						var _a;
						premium = (_a = token === null || token === void 0 ? void 0 : token.premium) !==
		...
		function onKeyChange(event) {
			...
			if (value.length === 5) {
				...
				CloudSDK.resolveToken(value).then((token) => {
					...
					premium = (_a = token === null || token === void 0 ? void 0 : token.premium) !== null &&
						_a !== void 0 ? _a : false;
				}).catch(() => {
					state = "nokey";
				});
	...
	function Key() {
		...
		return {
			...
			bind: (root) => {
				var _a;
				(_a = root.querySelector(".cloud-saves-clear")) === null || _a === void 0 ? void 0 : _a
